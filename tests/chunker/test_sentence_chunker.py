...
def test_sentence_chunker_repr(tokenizer):
    ...
    assert repr(chunker) == "SentenceChunker(chunk_size=512, chunk_overlap=128, mode='simple', min_sentences_per_chunk=1)"
...
def test_sentence_chunker_modes(tokenizer):
    ...
    # Test simple mode
    chunker_simple = SentenceChunker(tokenizer=tokenizer, chunk_size=512, chunk_overlap=128, mode="simple")
    assert chunker_simple.mode == "simple"
    
    # Test spacy mode
    chunker_spacy = SentenceChunker(tokenizer=tokenizer, chunk_size=512, chunk_overlap=128, mode="spacy")
...
def test_sentence_chunker_min_sentences(tokenizer):
    ...
    chunker = SentenceChunker(tokenizer=tokenizer, chunk_size=512, chunk_overlap=128, min_sentences_per_chunk=2)
